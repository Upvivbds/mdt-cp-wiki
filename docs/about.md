...
站点同时给出两个口径，因为游戏源码里的 `estimateDPS()` 把「一发打多个目标」
也算成了对单目标的输出，和实测对不上。两套分开算，页面上并排展示：

| 口径 | 含义 | 用途 |
| --- | --- | --- |
| **对单 DPS** | 不含多目标倍率，溅射取全额 | 排行榜默认口径，与实测吻合 |
| **群体 DPS** | 完整照搬 `BulletType.estimateDPS()`，含穿透 `×2`、激光 `×3`、闪电 `×长度/10`，溅射按 `0.75` 折算 | 看「一发能刷多少目标」的上限 |
...
差在龙王那门 `reload=0.5 / pierceCap=-1` 的炮——游戏模型给它的每一发都乘 2。
`LaserBulletType` 的自带注释写得很直白：`//assume it pierces at least 3 blocks`，
是个估算假设，不是实测值。
